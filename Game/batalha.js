...
async function batalha (vida_total, dano_total, defesaGuerreiro, vidaNPC, danoNPC, nomeNPC, idNPC, idGuerreiro) {
    ...
    if (vidaNPC <= 0) {
        try {
            const res = (`SELECT dinheiro, respeito FROM guerreiro WHERE idguerreiro=${idGuerreiro}`);
            var dinheiro = Number(res.rows[0].qtdrespeitobat);
            var respeito = Number(res.rows[0].qtddinheirobat);
        } catch (err) {
            console.log(err);
        }

        try {
            const res = (`SELECT qtdrespeitobat, qtddinheirobat FROM batalha WHERE idinimigo=${idNPC + 3}`);
            var qtdRespeito = Number(res.rows[0].qtdrespeitobat);
            var qtdDinheiro = Number(res.rows[0].qtddinheirobat);

            const newDinheiro = dinheiro + qtdDinheiro;
            const newRespeito = respeito + qtdRespeito;
            
            await db.query(`UPDATE guerreiro SET dinheiro = ${newDinheiro}, respeito = ${newRespeito} WHERE idguerreiro=${idGuerreiro}`);
        } catch (err) {
            console.log(err);
        }
        console.log('Você ganhou a batalha! \nDinheiro ganho: ' + qtdDinheiro + "\nRespeito ganho: " + qtdRespeito);
    } else {
...
async function getAtributos (idGuerreiro, idNPC) {
    var dano_total = 0, vida_total = 0, idItem = 0;
    ...
    try {
        await db.query(`INSERT INTO instancia_de_inimigo (idinimigo, idguerreiro) VALUES (${idNPC + 3}, ${idGuerreiro})`);
        const res = await db.query(`SELECT idinstancia FROM instancia_de_inimigo WHERE idguerreiro=${idGuerreiro}`);
    ...
    try {
        const res = await db.query(`SELECT vida, dano, nomenpc FROM inimigo WHERE idnpc=${idNPC + 3}`);
    ...
    try {
        const res = await db.query(`SELECT idbatalha FROM batalha WHERE idinimigo=${idNPC + 3}`);
    ...
    await batalha(vida_total, dano_total, defesaGuerreiro, vidaNPC, danoNPC, nomeNPC, idNPC, idGuerreiro);
