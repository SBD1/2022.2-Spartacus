...
async function missao_caverna(idGuerreiro) {
  try {
    const res = await db.query(
      `SELECT descricaomissao, nomemissao FROM missao WHERE idmissao=${14}`
    );
  ...
  try {
    const res = await db.query(
      `SELECT nomenpc,dano, vida FROM inimigo WHERE idnpc=${21}`
    );
...
async function missao_prespa(idGuerreiro) {
  try {
    const res = await db.query(
      `SELECT descricaomissao, nomemissao FROM missao WHERE idmissao=${19}`
    );
    console.log("MISSAO: " + res.rows[0].nomemissao);
    console.log(res.rows[0].descricaomissao);
  } catch (err) {
  ...
  try {
    const res = await db.query(
      `SELECT nomenpc,dano, vida FROM inimigo WHERE idnpc=${26}`
    );
    danoNPC = res.rows[0].dano;
    vidaNPC = res.rows[0].vida;
    nomeNPC = res.rows[0].nomenpc;
...
module.exports = { missao_caverna, missao_prespa };
